...
- USE WHEN the editor disagrees with `npx tsc` — trust CLI for merge gates; then check which LS is running (`useTsgo`, workspace `tsdk`).
...
“Restart the TS server / switch tsdk to fix a type error.” If `npx tsc` is red, the source is red. The LS is not a second type system you can vote against.
